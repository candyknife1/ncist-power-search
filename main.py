...
CORS(app)
api = 完美校园(phone_num='',password='',device_id='5745286925431029')
...
class server:
    ...
    def get_buildings():
        ...
        try:
            ...
            print("检查 data 目录中的 building_*.json 文件")
            
            # 检查是否有 building_*.json 文件
            building_files = list(glob.glob('data/building_*.json'))
            print(f"找到 {len(building_files)} 个 building_*.json 文件")
            
            if not building_files:
                print("未找到任何 building_*.json 文件，尝试初始化建筑数据")
                # 如果没有找到文件，初始化建筑数据
                web = server()
                web.init_building_data()
                building_files = list(glob.glob('data/building_*.json'))
                print(f"初始化后找到 {len(building_files)} 个 building_*.json 文件")
                
                if not building_files:
                    print("初始化后仍未找到任何 building_*.json 文件")
                    return jsonify({'error': '未找到任何楼栋数据，请联系管理员'}), 500
            ...
            for file_path in building_files:
                ...
                try:
                    ...
                    with open(file_path, 'r', encoding='utf-8') as f:
                        ...
                        print(f"文件内容类型: {type(building_data)}")
                        
                        # 检查 'name' 字段是否存在
                        if isinstance(building_data, dict) and 'name' in building_data:
                            buildings.append({
                                'name': building_data['name']
                            })
                            print(f"成功添加楼栋: {building_data['name']}")
                        else:
                            print(f"文件 {file_path} 中没有 'name' 字段或格式不正确")
                            print(f"文件内容: {building_data}")
                except Exception as e:
            ...
            if not buildings:
                print("未能成功读取任何楼栋数据")
                return jsonify({'error': '未能成功读取任何楼栋数据'}), 500
            
            print(f"成功读取 {len(buildings)} 个楼栋数据")
            return jsonify(buildings)
        except Exception as e:
            error_msg = f"获取楼栋列表时出错: {str(e)}"
            print(error_msg)
            import traceback
            traceback.print_exc()
    ...
    @app.route('/get_building_power', methods=['POST'])
