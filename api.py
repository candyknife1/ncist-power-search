...
class 完美校园:
    ...
    #             # self.pay_id = random.randint(118443,1110)
    ...
    def __get_fun(self, params, headers, url="https://h5cloud.17wanxiao.com:18443/CloudPayment/user/getRoom.do"):
        try:
            resp = requests.get(url=url,
                                params=params,
                                headers=headers)
            if resp.status_code == 200 and not ("-400" in resp.text):  # 移除 FAIL 相关判断
                return True, resp.json()
            return False, f"__get_fun请求失败{resp.status_code}"
        except Exception as e:
            # cookie过期，json()就会报错
    ...
    def get_part_id(self):
        ...
        headers = self.fun_headers.copy()
        headers['Referer'] = "https://h5cloud.17wanxiao.com:18443/CloudPayment/bill/selectPayProject.do"
        ...
        for i in update_list:
            ...
            if i == "room":
                ...
                for part in self.parts:
                    ...
                    # 查询当前公寓所有寝室ID
                    res,msg = self.get_room_list(part_id, 0)
                    if res:
                        try:
                            # 覆盖写入
    ...
    def init(self):
        ...
        # pay_id_list = [118443,1106,1108] # 订单号时好时坏，就这样吧
        print(f"当前订单号为：{self.pay_id}")
    ...
    def get_unitid(self,id):
        ...
        headers = self.fun_headers.copy()
        headers['Referer'] = f"https://h5cloud.17wanxiao.com:18443/CloudPayment/bill/selectPayProject.do"
        res,msg = self.__get_fun(params=params,headers=headers)
        if not res:
            return False,msg
        return True,msg
    ...
    # 修改获取房间列表方法
    def get_room_list(self, buildid, levelid):
        params = {
            'payProId': self.pay_id,
            'schoolcode': self.scool_code,
            'optype': '4',  # 获取房间列表
